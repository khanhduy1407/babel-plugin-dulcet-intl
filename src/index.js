...
export default function () {
    ...
    function getMessageDescriptorKey(path) {
        ...
        throw path.buildCodeFrameError(
            '[Dulcet Intl] Messages must be statically evaluate-able for extraction.'
        );
    }
                ...
                try {
                    ...
                    if (isJSXSource &&
                        ...
                        throw valuePath.buildCodeFrameError(
                            '[Dulcet Intl] Message failed to parse. ' +
                            'It looks like `\\`s were used for escaping, ' +
                            'this won\'t work with JSX string literals. ' +
                            'Wrap with `{}`. ' +
                            'See: http://khanhduy1407.github.io/dulcet/docs/jsx-gotchas.html'
                        );
                    }
    ...
    function storeMessage({id, description, defaultMessage}, path, state) {
        const {opts, dulcetIntl} = state;

        if (!(id && defaultMessage)) {
            throw path.buildCodeFrameError(
                '[Dulcet Intl] Message Descriptors require an `id` and `defaultMessage`.'
            );
        }

        if (dulcetIntl.messages.has(id)) {
            let existing = dulcetIntl.messages.get(id);

            if (description !== existing.description ||
                defaultMessage !== existing.defaultMessage) {

                throw path.buildCodeFrameError(
                    `[Dulcet Intl] Duplicate message id: "${id}", ` +
        ...
        if (opts.enforceDescriptions && !description) {
            throw path.buildCodeFrameError(
                '[Dulcet Intl] Message must have a `description`.'
            );
        }

        dulcetIntl.messages.set(id, {id, description, defaultMessage});
    }
    ...
    return {
        visitor: {
            Program: {
                ...
                exit(path, state) {
                    const {file, opts, dulcetIntl} = state;
                    const {basename, filename}    = file.opts;

                    let descriptors = [...dulcetIntl.messages.values()];
            ...
            JSXOpeningElement(path, state) {
                ...
                if (name.referencesImport(moduleSourceName, 'FormattedPlural')) {
                    file.log.warn(
                        `[Dulcet Intl] Line ${path.node.loc.start.line}: ` +
            ...
            CallExpression(path, state) {
                ...
                function assertObjectExpression(node) {
                    if (!(node && node.isObjectExpression())) {
                        throw path.buildCodeFrameError(
                            `[Dulcet Intl] \`${callee.node.name}()\` must be ` +
                            'called with an object expression with values ' +
                            'that are Dulcet Intl Message Descriptors, also ' +
                ...
                function processMessageObject(messageObj) {
                    ...
                    if (!descriptor.defaultMessage) {
                        throw path.buildCodeFrameError(
                            '[Dulcet Intl] Message is missing a `defaultMessage`.'
                        );
                    }
